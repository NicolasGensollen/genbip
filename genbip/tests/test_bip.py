...
def test_raw_instantiation():
    test_bip = bip("","",[1,3,2,4],[2,5,1,1,1],["a","b","c","d"],["1","2","3","4","5"])
    assert test_bip.top_filename == ""
    assert test_bip.bot_filename == ""
    assert test_bip.top_degree == [1,3,2,4]
    assert test_bip.bot_degree == [2,5,1,1,1]
    assert test_bip.top_index == []
    assert test_bip.top_vector == []
    assert test_bip.bot_vector == []
...
def test_instantiation_from_files_toy_1():
    ...
    assert toy1_bip.top_index == []
    assert toy1_bip.top_vector == []
    assert toy1_bip.bot_vector == []
...
def test_instantiation_from_sequences():
    ...
    assert from_seq_bip.top_index == []
    assert from_seq_bip.top_vector == []
